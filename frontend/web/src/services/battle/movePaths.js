...
module.exports = class MovePaths {
    ...
    getReachables() {
        ...
        for (let step = 0; step < this.movement; step++) {
            lastSteps = lastSteps.reduce((nextSteps, lastStep) => {
                let nextPointDirections = [[0, -1], [-1, 0], [1, 0], [0, 1]]
                nextPointDirections = lastStep.y % 2 ? [...nextPointDirections, [1, 1], [1, -1]] : [...nextPointDirections, [-1, 1], [-1, -1]]

                nextPointDirections.map(nextPointDirection => {
                    let nextStep = {
                        x: lastStep.x + nextPointDirection[0],
                        y: lastStep.y + nextPointDirection[1],
                    };
    ...
    getPaths(destination) {
        ...
        if (!this.reachables) this.reachables = this.getReachables()
